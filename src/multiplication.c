/*
  ...
  Title: Matrix multiplication program for exercise 3.
...
void cleanup(tipo **a, tipo **b, tipo **c) {
  freeMatrix(a);
  freeMatrix(b);
  freeMatrix(c);
}

// Function that implements the matrix multiplication, and whose execution time will be recorded
void multiply(tipo **res, tipo **a, tipo **b, int dim) {
  int i, j, k;
  for(i = 0; i < dim; i++) {
    for(j = 0; j < dim; j++) {
      for(k = 0; k < dim; k++) {
